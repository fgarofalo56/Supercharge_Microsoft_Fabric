...
class VideoAnalyticsGenerator(BaseGenerator):
    ...
    def generate_record(self) -> dict[str, Any]:
        ...
        camera = self._cameras[int(np.random.randint(0, len(self._cameras)))]
        ...
        if event_type == "crowd_density":
            object_count = int(np.random.randint(1, 16))
        elif event_type != "anomaly":
            object_count = int(np.random.randint(1, 4))

        # Bounding box (None 30% of the time)
        bounding_box: dict[str, int] | None = None
        if np.random.random() >= 0.30:
            bounding_box = {
                "x": int(np.random.randint(0, 1921)),
                "y": int(np.random.randint(0, 1081)),
                "width": int(np.random.randint(20, 401)),
                "height": int(np.random.randint(20, 601)),
            }

        # Track ID (present for object_detection and zone_crossing)
        track_id: str | None = None
        if event_type in ("object_detection", "zone_crossing"):
            track_hex = format(np.random.randint(0, 0x7FFFFFFF), "08x")
        ...
        if event_type == "loitering":
            dwell_time_seconds = round(float(np.random.uniform(30.0, 600.0)), 1)
        elif event_type == "crowd_density":
            dwell_time_seconds = round(float(np.random.uniform(10.0, 300.0)), 1)
        ...
        major = int(np.random.randint(1, 4))
        minor = int(np.random.randint(0, 10))
        patch = int(np.random.randint(0, 20))
