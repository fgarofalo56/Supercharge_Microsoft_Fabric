...
class TribalHealthcareGenerator(BaseGenerator):
    ...
    def generate_record(self) -> dict[str, Any]:
        ...
        # --- Facility selection ---
        facility = FACILITIES[int(np.random.randint(0, len(FACILITIES)))]

        # --- Diagnosis ---
        icd10_idx = int(np.random.choice(len(ICD10_CODES), p=ICD10_WEIGHTS))
        ...
        if np.random.random() < 0.75:
            cpt_idx = int(np.random.choice(len(CPT_CODES), p=CPT_WEIGHTS))
        ...
        if np.random.random() < 0.85:
            npi_num = int(np.random.uniform(1_000_000_000, 9_999_999_999))
        ...
        if age_group == "65+" and np.random.random() < 0.60:
            insurance = "MEDICARE"
        elif age_group in ("0-4", "5-14") and np.random.random() < 0.50:
            insurance = "MEDICAID"

        # --- Patient ID ---
        patient_hash = self.hash_value(
            f"{tribal}-{gender}-{age_group}-{np.random.randint(0, 999999)}"
        )[:8].upper()
        ...
        if encounter_type == "pharmacy" or np.random.random() < 0.30:
            med = MEDICATIONS[int(np.random.randint(0, len(MEDICATIONS)))]
        ...
        if encounter_type == "laboratory" or np.random.random() < 0.20:
            lab = LAB_TESTS[int(np.random.randint(0, len(LAB_TESTS)))]
            lab_test_name = lab["name"]
            lab_result_value = round(
                float(np.random.uniform(lab["low"], lab["high"])), 2
            )
