...
_MAG_TYPE_SMALL  = np.array([0.50, 0.20, 0.12, 0.06, 0.04, 0.03, 0.03, 0.02], dtype=float)
_MAG_TYPE_MEDIUM = np.array([0.20, 0.10, 0.15, 0.25, 0.10, 0.10, 0.05, 0.05], dtype=float)
_MAG_TYPE_LARGE  = np.array([0.02, 0.02, 0.06, 0.30, 0.10, 0.25, 0.15, 0.10], dtype=float)

# ── Event type ──────────────────────────────────────────────────────────────
_EVENT_TYPES   = ["EARTHQUAKE", "QUARRY_BLAST", "EXPLOSION", "VOLCANIC_ERUPTION", "ICE_QUAKE"]
...
def _pick_seismic_location() -> tuple[float, float]:
    """Return (latitude, longitude) from a randomly selected seismic zone."""
    idx = int(np.random.choice(len(_SEISMIC_ZONES), p=_ZONE_WEIGHTS))
...
def _pick_magnitude() -> float:
    """Return a magnitude sampled from the Gutenberg-Richter distribution."""
    bucket_idx = int(np.random.choice(len(_MAG_BUCKETS), p=_MAG_PROBS))
    mag_min, mag_max, _ = _MAG_BUCKETS[bucket_idx]
    # Uniform within bucket approximates the GR law at this coarse resolution.
    return round(float(np.random.uniform(mag_min, mag_max)), 1)


def _pick_mag_type(magnitude: float) -> str:
    ...
    return str(np.random.choice(_MAG_TYPES, p=w))


def _pick_alert(magnitude: float) -> str | None:
    """Return PAGER alert level correlated with magnitude."""
    if magnitude < 5.0:
        # ~95 % null, ~5 % GREEN
        return None if np.random.random() < 0.95 else "GREEN"
    elif magnitude < 6.0:
    ...
    return _ALERTS[int(np.random.choice(len(_ALERTS), p=np.array(weights, dtype=float)))]


def _significance(magnitude: float) -> int:
    ...
    jitter = int(np.random.randint(-30, 31))
...
_STATES_LAND: list[tuple[str, float]] = [
    ("NV", 0.10), ("UT", 0.09), ("AZ", 0.08), ("NM", 0.07), ("WY", 0.07),
    ("MT", 0.07), ("ID", 0.06), ("OR", 0.06), ("AK", 0.06), ("CO", 0.06),
    ("CA", 0.06), ("WA", 0.04), ("ND", 0.03), ("SD", 0.03), ("NE", 0.02),
    ("KS", 0.02), ("OK", 0.02), ("TX", 0.02), ("MN", 0.01), ("WI", 0.01),
    ("MI", 0.01), ("VA", 0.01), ("FL", 0.01),
]
...
_AGENCY_LAND_TYPES: dict[str, list[tuple[str, float]]] = {
    "BLM":   [("GRAZING", 0.40), ("MINING", 0.25), ("MIXED_USE", 0.15),
               ("RECREATION", 0.10), ("WILDERNESS", 0.05), ("TIMBER", 0.03), ("CONSERVATION", 0.02)],
    "NPS":   [("RECREATION", 0.50), ("WILDERNESS", 0.25), ("CONSERVATION", 0.15),
               ("MIXED_USE", 0.08), ("GRAZING", 0.01), ("MINING", 0.01), ("TIMBER", 0.00)],
    "FWS":   [("CONSERVATION", 0.55), ("WILDERNESS", 0.20), ("RECREATION", 0.10),
               ("MIXED_USE", 0.08), ("GRAZING", 0.05), ("TIMBER", 0.01), ("MINING", 0.01)],
    "USFS":  [("TIMBER", 0.40), ("RECREATION", 0.25), ("WILDERNESS", 0.15),
               ("GRAZING", 0.10), ("MIXED_USE", 0.07), ("CONSERVATION", 0.02), ("MINING", 0.01)],
    "BOR":   [("RECREATION", 0.35), ("CONSERVATION", 0.25), ("MIXED_USE", 0.25),
               ("GRAZING", 0.10), ("WILDERNESS", 0.03), ("TIMBER", 0.01), ("MINING", 0.01)],
    "DOD":   [("MIXED_USE", 0.50), ("RECREATION", 0.20), ("GRAZING", 0.15),
               ("CONSERVATION", 0.08), ("WILDERNESS", 0.05), ("TIMBER", 0.01), ("MINING", 0.01)],
    "OTHER": [("MIXED_USE", 0.40), ("RECREATION", 0.20), ("GRAZING", 0.15),
               ("CONSERVATION", 0.10), ("MINING", 0.08), ("TIMBER", 0.05), ("WILDERNESS", 0.02)],
}

# ── Permit types ─────────────────────────────────────────────────────────────
_PERMIT_TYPES         = ["GRAZING_PERMIT", "MINING_CLAIM", "SPECIAL_USE",
                          "RIGHT_OF_WAY", "NONE", None]
...
_DESIGNATIONS: dict[str, list[str]] = {
    "BLM":  ["National Monument", "National Conservation Area", "Wilderness Study Area",
              "Special Recreation Management Area", None, None],
    "NPS":  ["National Park", "National Monument", "National Seashore",
              "National Recreation Area", "National Historic Site"],
    "FWS":  ["National Wildlife Refuge", "Wilderness Area", "Critical Habitat",
              "Waterfowl Production Area", None],
    "USFS": ["National Forest", "Wilderness Area", "National Grassland",
              "Special Interest Area", None],
    "BOR":  ["National Recreation Area", "Reservoir Management Area",
              "Water Project Area", None, None],
...
def _pick_land_location(state: str) -> tuple[float, float]:
    """Return US (latitude, longitude) weighted toward western states."""
    if state in _WESTERN_STATES:
        lat = float(np.random.uniform(24.0, 71.5))
        lon = float(np.random.uniform(-168.0, -104.0))
    else:
        lat = float(np.random.uniform(24.0, 49.0))
        lon = float(np.random.uniform(-104.0, -66.9))
    return round(lat, 6), round(lon, 6)


def _pick_land_type(agency: str) -> str:
...
class DOIGenerator(BaseGenerator):
    ...
    def _generate_earthquake_record(self) -> dict[str, Any]:
        ...
        magnitude = _pick_magnitude()
        mag_type  = _pick_mag_type(magnitude)
        lat, lon  = _pick_seismic_location()

        # Depth: most shallow (0-30 km), long tail to 700 km
        depth_roll = np.random.random()
        if depth_roll < 0.70:
            depth_km = round(float(np.random.uniform(0.0, 30.0)), 2)
        elif depth_roll < 0.90:
            depth_km = round(float(np.random.uniform(30.0, 100.0)), 2)
        else:
            depth_km = round(float(np.random.uniform(100.0, 700.0)), 2)
        ...
        # ── place description ────────────────────────────────────────────────
        dist_km  = int(np.random.randint(2, 100))
        direction = str(np.random.choice(_DIRECTIONS))
        ...
        event_type = str(np.random.choice(_EVENT_TYPES, p=_EVENT_WEIGHTS))
        status     = str(np.random.choice(_STATUSES, p=_STATUS_WEIGHTS))

        # ── tsunami flag: shallow and large only ─────────────────────────────
        tsunami = bool(magnitude >= 6.5 and depth_km <= 70.0 and np.random.random() < 0.40)

        # ── felt reports (DYFI): only M2+ and only some fraction ─────────────
        felt: int | None = None
        cdi:  float | None = None
        if magnitude >= 2.0 and np.random.random() < 0.35:
            felt = int(np.random.randint(1, max(2, int(10 ** (magnitude - 1.5)))))
            cdi  = round(float(np.random.uniform(1.0, min(12.0, magnitude * 1.6))), 1)

        # ── ShakeMap MMI ──────────────────────────────────────────────────────
        mmi: float | None = None
        if magnitude >= 3.0 and np.random.random() < 0.60:
            mmi = round(float(np.random.uniform(1.0, min(12.0, magnitude * 1.5))), 1)

        # ── PAGER alert level ─────────────────────────────────────────────────
        alert = _pick_alert(magnitude)

        # ── quality / network parameters ──────────────────────────────────────
        sig   = _significance(magnitude)
        net   = str(np.random.choice(_NETWORKS))
        nst   = int(np.random.randint(5, 501))
        gap   = round(float(np.random.uniform(10.0, 360.0)), 1)
        rms   = round(float(np.random.uniform(0.01, 2.0)), 3)
    ...
    def _generate_land_use_record(self) -> dict[str, Any]:
        ...
        state = str(np.random.choice(_STATE_ABBRS_LAND, p=_STATE_WEIGHTS_LAND))
        agency = str(np.random.choice(_AGENCIES, p=_AGENCY_WEIGHTS))
        lat, lon = _pick_land_location(state)

        # ── land classification ───────────────────────────────────────────────
        land_type = _pick_land_type(agency)
        total_acres = _log_normal_acres()

        # ── identifiers ──────────────────────────────────────────────────────
        parcel_id = self.generate_uuid()
        blm_serial: str | None = None
        if agency in ("BLM", "BOR") and np.random.random() < 0.80:
            seq = f"{int(np.random.randint(100000, 999999))}"
            blm_serial = f"BLM-{state}-{seq}"

        # ── county (optional, 75 %) ───────────────────────────────────────────
        county: str | None = self.faker.city() if np.random.random() < 0.75 else None

        # ── designation ───────────────────────────────────────────────────────
        desig_pool = _DESIGNATIONS.get(agency, [None])
        designation: str | None = str(np.random.choice(desig_pool))
        if designation == "None":
            designation = None
        ...
        if permit_holder is not None and np.random.random() < 0.70:
            annual_revenue = round(float(np.random.uniform(0.0, 10_000_000.0)), 2)

        # ── environmental / ecological flags ─────────────────────────────────
        env_assess: bool | None = None
        if np.random.random() < 0.80:
            env_assess = bool(np.random.random() < 0.55)
        ...
        if np.random.random() < 0.75:
            ...
            protected_species = bool(np.random.random() < prob)
        ...
        if np.random.random() < 0.85:
            ...
            w = weights / weights.sum()
            fire_risk = str(np.random.choice(_FIRE_RISK, p=w))
            if fire_risk == "None":
                fire_risk = None
