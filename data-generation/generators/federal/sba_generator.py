...
class SBAGenerator(BaseGenerator):
    ...
    def generate_record(self, domain: str = "ppp") -> dict[str, Any]:  # type: ignore[override]
        ...
        naics_code = str(np.random.choice(NAICS_LIST))
        ...
        borrower_city: str | None = (
            self.faker.city() if np.random.random() > 0.05 else None
        )
    ...
    def _generate_loan_amount(self, domain: str) -> float:
        ...
        if domain == "ppp":
            ...
            if tier < 0.55:
                ...
                return round(np.random.uniform(20_000, 150_000), 2)
            elif tier < 0.80:
                # Mid-size: $150K-$500K
                return round(np.random.uniform(150_000, 500_000), 2)
            elif tier < 0.95:
                # Larger: $500K-$2M
                return round(np.random.uniform(500_000, 2_000_000), 2)
            else:
                # Max band: $2M-$10M
                return round(np.random.uniform(2_000_000, 10_000_000), 2)

        elif domain == "7a":
            tier = np.random.random()
            if tier < 0.40:
                return round(np.random.uniform(5_000, 150_000), 2)
            elif tier < 0.75:
                return round(np.random.uniform(150_000, 1_000_000), 2)
            else:
                return round(np.random.uniform(1_000_000, 5_000_000), 2)

        elif domain == "disaster":
            return round(np.random.uniform(1_000, 2_000_000), 2)

        else:  # sbir
            tier = np.random.random()
            if tier < 0.60:
                # Phase I awards ~$50K-$250K
                return round(np.random.uniform(50_000, 250_000), 2)
            else:
                # Phase II awards ~$250K-$1.5M
                return round(np.random.uniform(250_000, 1_500_000), 2)
    ...
    def _generate_jobs_retained(self) -> int:
        ...
        if tier < 0.10:
            ...
            return int(np.random.randint(1, 25))
        elif tier < 0.92:
            return int(np.random.randint(25, 100))
        elif tier < 0.99:
            return int(np.random.randint(100, 300))
        else:
            return int(np.random.randint(300, 501))
    ...
    def _generate_interest_rate(self, domain: str, cfg: dict[str, Any]) -> float:
        ...
        return round(np.random.uniform(rate_min, rate_max), 2)
    ...
    def _generate_forgiveness(
        ...
        if roll < 0.88:
            ...
            forgiveness_amount = round(
                np.random.uniform(loan_amount * 0.50, loan_amount * 0.99), 2
            )
        else:
        ...
        weeks_offset = np.random.randint(8, 25)
