...
class MultiSourceSimulator(BaseGenerator):
    ...
    def generate_record(self, source_type: str = "sql_server") -> dict[str, Any]:  # type: ignore[override]
        ...
        if cfg["pk_style"] == "uuid":
            ...
            primary_key = str(np.random.randint(100_000, 9_999_999))
        ...
        txn_id: str | None
        if operation == "READ" or np.random.random() < 0.10:
            txn_id = None
        else:
        ...
        latency_ms = int(
            np.random.randint(cfg["latency_range"][0], cfg["latency_range"][1])
        )
    ...
    def _generate_casino_image(self) -> dict[str, Any]:
        ...
        pool: dict[str, Any] = {
            "player_id": "P" + str(np.random.randint(100_000, 999_999)),
            "amount": round(float(np.random.uniform(1.0, 50_000.0)), 2),
    ...
    def _generate_lsn(self) -> str:
        ...
        """
        seg1 = format(np.random.randint(0x00000001, 0x0000FFFF), "08x")
        seg2 = format(np.random.randint(0x00000001, 0x0000FFFF), "08x")
        seg3 = format(np.random.randint(0x0001, 0x00FF), "04x")
    ...
    def _generate_partition_key(self, cfg: dict[str, Any]) -> str:
        ...
        numeric_id = str(np.random.randint(10_000, 999_999))
