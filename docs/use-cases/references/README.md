...
- **CSV/JSON**: Directly ingestible via PySpark `spark.read.csv()` / `spark.read.json()` in Fabric notebooks
- **API**: Use `requests` library in Bronze notebooks; paginate and write to Delta tables
- **PDF**: Requires extraction (tabula-py or manual) before ingestion — suitable for reference documentation rather than automated pipelines
- **SAS/SPSS**: Convert via `pandas.read_sas()` or `pyreadstat` before loading to lakehouse
