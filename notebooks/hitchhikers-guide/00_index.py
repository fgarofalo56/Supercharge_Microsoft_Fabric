...
# MAGIC 4. The snippets favor **`notebookutils.*`** — the legacy
...
# MAGIC |---|---|---|---|
# MAGIC | 00 | `00_index.py` (this file) | Everyone | TOC + conventions |
# MAGIC | 01 | `01_connectivity.py` | Data engineers, integration leads | ADLS Gen2, S3, GCS, on-prem SQL, Snowflake, Synapse, Databricks, Warehouse, SQL DB, Lakehouse SQL endpoint, Eventhouse, GraphQL, Power BI semantic models, Fabric REST, Cosmos, PostgreSQL, MySQL |
# MAGIC | 02 | `02_lakehouse_warehouse_ops.py` | Data engineers | Delta reads/writes, OneLake addressing, default lakehouse, lakehouse CRUD, Warehouse from Spark, schemas, partitions, MERGE, OPTIMIZE/Z-ORDER/VACUUM |
# MAGIC | 03 | `03_security_identity.py` | Security engineers, admins | OneLake RLS/CLS, Warehouse RLS/CLS/DDM, semantic model RLS via DAX, workspace identity & SPN tokens, Key Vault secrets, MSAL |
# MAGIC | 04 | `04_admin_governance.py` | Platform admins | Workspace/lakehouse/warehouse REST, role assignments, Git integration, deployment pipelines, tenant settings, semantic model refresh, capacity assignment, workspace monitoring KQL |
# MAGIC | 05 | `05_automation_utilities.py` | Automation engineers | `notebookutils` fs/credentials/notebook/runtime, parallel DAG runs, parameter cells, public Run-Notebook API |
# MAGIC | 06 | `06_troubleshooting.py` | Everyone (eventually) | Symptom→cause→fix matrix with links |
# MAGIC
# MAGIC ## Conventions
# MAGIC
