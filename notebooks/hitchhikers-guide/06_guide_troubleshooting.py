...
# MAGIC | `notebookutils.notebook.exit()` silently ignored | Called inside try/except, or pipeline misreads exit value | Move `exit()` out of exception handlers; check pipeline configuration |
...
# MAGIC | `notebookutils.fs.cp` is slow against S3/GCS shortcut | abfss path hits the control plane every hop | Mount the shortcut path instead |
