...
# MAGIC ✅ `notebookutils.*` — 2026 canonical
# MAGIC ❌ `dbutils.*` — does NOT exist in Fabric
# MAGIC
# MAGIC | Databricks | Fabric equivalent |
# MAGIC |---|---|
# MAGIC | `dbutils.fs.ls` | `notebookutils.fs.ls` |
# MAGIC | `dbutils.notebook.run` | `notebookutils.notebook.run` |
# MAGIC | `dbutils.secrets.get` | `notebookutils.credentials.getSecret` |
...
notebookutils.fs.mkdirs("Files/raw/2026/05")
notebookutils.fs.ls("Files/raw")
notebookutils.fs.cp("Files/in.csv", "Files/archive/in.csv", recurse=False)
notebookutils.fs.mv("Files/src.csv", "Files/done/src.csv", create_path=True, overwrite=True)
notebookutils.fs.put("Files/marker.txt", "ok", overwrite=True)
notebookutils.fs.head("Files/marker.txt", maxBytes=100)
notebookutils.fs.fastcp(src="abfss://...", dst="abfss://...", recurse=True)  # azcopy
...
if not any(m.mountPoint == "/data" for m in existing):
    notebookutils.fs.mount("abfss://c@acct.dfs.core.windows.net", "/data")

local = notebookutils.fs.getMountPath("/data")
print(local)  # use as if it were a local path

notebookutils.fs.unmount("/data")
...
secret = notebookutils.credentials.getSecret("https://kv.vault.azure.net/", "name")
notebookutils.credentials.putSecret("https://kv.vault.azure.net/", "name", "value")
...
kv_token      = notebookutils.credentials.getToken("keyvault")
...
result = notebookutils.notebook.run("ChildNotebook", 90, {"input": 20})
...
results = notebookutils.notebook.runMultiple(spec)
print(results)
...
# MAGIC `notebookutils.notebook.run(..., args)` time. ⚠️ Only int / float /
...
# MAGIC ## Exit value
# MAGIC
# MAGIC Use `notebookutils.notebook.exit("...")` to return a value to the
