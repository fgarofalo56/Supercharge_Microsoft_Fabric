...
def poll(operation_url: str, timeout_s: int = 600):
    """Long-running operations return 202 + Location header."""
    start = time.time()
    while True:
...
requests.post(
  f"https://api.powerbi.com/v1.0/myorg/groups/<workspace-id>/datasets/<dataset-id>/refreshes",
...
# XMLA connection string (for Tabular Editor / SSMS / scripts):
xmla_cs = (
    "Data Source=powerbi://api.powerbi.com/v1.0/myorg/<workspace>;"
    "Initial Catalog=<dataset>;"
    "User ID=app:<appId>@<tenantId>;Password=<secret>;"
)
