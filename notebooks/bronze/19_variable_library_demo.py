...
environment = spark.conf.get("spark.fabric.variable.environment", "dev")
landing_path = spark.conf.get(
    "spark.fabric.variable.landing_path",
    "Files/landing/slot_telemetry/",
)
target_table = spark.conf.get(
    "spark.fabric.variable.target_table",
    "bronze_slot_telemetry",
)
lakehouse_name = spark.conf.get(
    "spark.fabric.variable.lakehouse_name",
    "lh_bronze",
)
batch_id = spark.conf.get(
    "spark.fabric.variable.batch_id",
...
source_path = f"abfss://{lakehouse_name}@onelake.dfs.fabric.microsoft.com/{landing_path}"
...
# MAGIC
# MAGIC # Fabric REST API — create variable library
