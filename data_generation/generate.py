...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        "--days", "-d", type=int, default=30, help="Days of history (default: 30)"
    )
    p.add_argument(
    ...
    p.add_argument(
        ...
        "-a",
        action="store_true",
        help="Generate every casino data type at default volumes",
    )
    ...
    p.add_argument(
        "--include-pii", action="store_true", help="Include unhashed PII (testing only)"
    )

    # Federal generators
    federal_choices = (
        ",".join(sorted(FEDERAL_GENERATORS))
        if FEDERAL_GENERATORS
        else "(none installed)"
    )
    p.add_argument(
        "--federal",
    ...
    p.add_argument(
        ...
        type=int,
        default=5000,
        help="Records per federal generator (default: 5000)",
    )

    # Analytics generators
    analytics_choices = (
        ",".join(sorted(ANALYTICS_GENERATORS))
        if ANALYTICS_GENERATORS
        else "(none installed)"
    )
    p.add_argument(
        "--analytics",
    ...
    p.add_argument(
        ...
        type=int,
        default=10000,
        help="Records per analytics generator (default: 10000)",
    )
...
def _emit(
    name: str, gen, count: int, output_dir: Path, fmt: str, summary: list
) -> None:
...
def generate_data(args: argparse.Namespace) -> None:
    ...
    gen_casino(
        ...
        SlotMachineGenerator,
        args.slots,
        default_casino_volumes["slots"],
    )
    gen_casino(
        "table_games", TableGameGenerator, args.tables, default_casino_volumes["tables"]
    )
    gen_casino(
        "player_profile",
        PlayerGenerator,
        args.players,
        default_casino_volumes["players"],
    ...
    gen_casino(
        ...
        FinancialGenerator,
        args.financial,
        default_casino_volumes["financial"],
    )
    gen_casino(
        "security_events",
        SecurityGenerator,
        args.security,
        default_casino_volumes["security"],
    )
    gen_casino(
        "compliance_filings",
        ComplianceGenerator,
        args.compliance,
        default_casino_volumes["compliance"],
    )
    ...
    if federal_subset and not FEDERAL_AVAILABLE:
        print(
            "ERROR: --federal selected but federal generators not installed",
            file=sys.stderr,
        )
        sys.exit(2)
    for key in federal_subset:
        if key not in FEDERAL_GENERATORS:
            print(
                f"WARNING: unknown federal generator {key!r}; available: {sorted(FEDERAL_GENERATORS)}",
                file=sys.stderr,
            )
            continue
        ...
        _emit(
            stem,
            cls(seed=args.seed, start_date=start_date, end_date=end_date),
            args.federal_records,
            output_dir,
            args.format,
            summary,
        )
    ...
    if analytics_subset and not ANALYTICS_AVAILABLE:
        print(
            "ERROR: --analytics selected but analytics generators not installed",
            file=sys.stderr,
        )
        sys.exit(2)
    for key in analytics_subset:
        if key not in ANALYTICS_GENERATORS:
            print(
                f"WARNING: unknown analytics generator {key!r}; available: {sorted(ANALYTICS_GENERATORS)}",
                file=sys.stderr,
            )
            continue
