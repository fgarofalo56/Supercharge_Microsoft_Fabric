...
class IoTDeviceSimulator(BaseGenerator):
    ...
    def _generate_devices_of_type(
        ...
        for n in range(1, count + 1):
            ...
            firmware_version = (
                f"v{self.rng.integers(1, 4)}"
                f".{self.rng.integers(0, 9)}"
                f".{self.rng.integers(0, 9)}"
            )
            devices.append(
                {
                    ...
                    "location_zone": str(self.rng.choice(zones)),
    ...
    def generate_record(self, device_type: str = "slot_machine") -> dict[str, Any]:
        ...
        enqueued_offset = int(self.rng.uniform(50, 2000))  # ms
        ...
        correlation_id = self.generate_uuid() if self.rng.random() < 0.40 else None
        ...
        record: dict[str, Any] = {
            ...
            "signal_strength_dbm": int(self.rng.integers(-90, -29)),
        }
    ...
    def generate_fleet_snapshot(self) -> list[dict[str, Any]]:
        ...
        for dtype, devices in self._fleet.items():
            for device in devices:
                ...
                enqueued_offset = int(self.rng.uniform(50, 500))
                enqueued_time = timestamp + timedelta(milliseconds=enqueued_offset)

                properties = self._make_properties(dtype, device)
                correlation_id = (
                    self.generate_uuid() if self.rng.random() < 0.40 else None
                )
    ...
    def _telemetry_slot_machine(self, device: dict[str, Any]) -> dict[str, Any]:
        ...
        error_code = None
        if status == "ERROR":
            error_code = f"E{self.rng.integers(1, 8):03d}"

        denomination = float(
            self.rng.choice([0.01, 0.05, 0.25, 0.50, 1.00, 2.00, 5.00])
        )
        coin_in = round(float(self.rng.uniform(10_000, 1_000_000)), 2)
        hold = self.rng.uniform(0.04, 0.12)
        coin_out = round(coin_in * (1 - hold), 2)

        return {
            "machine_number": device["device_id"].split("-")[-1],
            "denomination": denomination,
            "game_id": str(self.rng.choice(_SLOT_GAME_IDS)),
            "coin_in_meter": coin_in,
            "coin_out_meter": coin_out,
            "games_played_meter": int(coin_in / denomination / 3),
            "jackpot_meter": round(float(self.rng.uniform(0, 50_000)), 2),
            "bill_in_meter": round(float(self.rng.uniform(5_000, 500_000)), 2),
            "status": status,
    ...
    def _telemetry_table_sensor(self, device: dict[str, Any]) -> dict[str, Any]:
        ...
        players_seated = int(self.rng.integers(0, 9))
        return {
            "table_id": device["device_id"],
            "game_type": str(self.rng.choice(_TABLE_GAME_TYPES)),
            "players_seated": players_seated,
            "chips_on_table": round(float(self.rng.uniform(0, 50_000)), 2),
    ...
    def _telemetry_hvac_sensor(self, device: dict[str, Any]) -> dict[str, Any]:
        ...
        return {
            ...
            "temperature": round(float(self.rng.uniform(65.0, 80.0)), 1),
            "humidity": round(float(self.rng.uniform(30.0, 70.0)), 1),
            "co2_level": int(self.rng.integers(400, 2001)),
            "air_flow_cfm": round(float(self.rng.uniform(200.0, 2000.0)), 1),
        }

    def _telemetry_door_sensor(self, device: dict[str, Any]) -> dict[str, Any]:
        """Door access-control event."""
        badge_id = f"BADGE-{self.rng.integers(10000, 99999)}"
        ...
        return {
            ...
            "direction": str(self.rng.choice(["IN", "OUT"])),
            "badge_id": badge_id,
            "access_granted": bool(self.rng.random() < grant_rate),
            "area": area,
        }

    def _telemetry_camera(self, device: dict[str, Any]) -> dict[str, Any]:
        """Video analytics frame metadata."""
        objects_detected = int(self.rng.integers(0, 20))
        return {
            "camera_id": device["device_id"],
            "frame_count": int(self.rng.integers(1, 10_000_001)),
            "objects_detected": objects_detected,
            "motion_detected": bool(objects_detected > 0 and self.rng.random() < 0.80),
            "recording": True,
        }

    def _telemetry_beacon(self, device: dict[str, Any]) -> dict[str, Any]:
        """BLE beacon proximity signal."""
        rssi = int(self.rng.integers(-100, -29))
        ...
        return {
            ...
            "major": int(self.rng.integers(1, 65536)),
            "minor": int(self.rng.integers(1, 65536)),
            "rssi": rssi,
    ...
    def _telemetry_environmental(self, device: dict[str, Any]) -> dict[str, Any]:
        ...
        return {
            ...
            "noise_level_db": round(float(self.rng.uniform(40.0, 100.0)), 1),
            "light_level_lux": round(float(self.rng.uniform(50.0, 1000.0)), 1),
            "smoke_detected": bool(self.rng.random() < 0.005),
        }
