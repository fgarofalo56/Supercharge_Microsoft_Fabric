...
class NOAAGenerator(BaseGenerator):
    ...
    def _generate_weather_record(self) -> dict[str, Any]:
        ...
        station_code: str = str(self.rng.choice(STATION_CODES))
        station: dict[str, Any] = STATIONS[station_code]

        timestamp: datetime = self.random_datetime()
        parameter: str = str(self.rng.choice(PARAMETERS))
        ...
        data_source: str = str(self.rng.choice(DATA_SOURCES))
    ...
    def _sample_parameter_value(self, parameter: str) -> float:
        ...
        if dist == "normal":
            val = self.rng.normal(cfg["mean"], cfg["std"])
        elif dist == "gamma":
            val = self.rng.gamma(cfg["shape"], cfg["scale"])
        elif dist == "exponential":
            val = self.rng.exponential(cfg["scale"])
        else:  # uniform
    ...
    def _generate_storm_record(self) -> dict[str, Any]:
        ...
        episode_id: str | None = (
            f"EP-{self.generate_uuid()[:8]}" if self.rng.random() < 0.70 else None
        )
        ...
        county_fips: str | None = (
            f"{state_fips}{self.rng.integers(1, 200):03d}"
            if self.rng.random() < 0.80
            else None
        )

        begin_date: datetime = self.random_datetime()
        duration_hours: float = float(self.rng.exponential(scale=6.0))
        ...
        begin_lat: float = round(float(self.rng.uniform(25.0, 49.0)), 4)
        begin_lon: float = round(float(self.rng.uniform(-125.0, -67.0)), 4)
        end_lat: float = round(begin_lat + float(self.rng.normal(0, 0.5)), 4)
        end_lon: float = round(begin_lon + float(self.rng.normal(0, 0.5)), 4)
        ...
        if event_type in ("FLASH_FLOOD", "FLOOD"):
            flood_cause = str(self.rng.choice(FLOOD_CAUSES))

        source: str = str(self.rng.choice(STORM_SOURCES))
    ...
    def _sample_casualties(self, p_nonzero: float, max_val: int) -> int:
        ...
        if self.rng.random() > p_nonzero:
            return 0
        return int(np.clip(self.rng.exponential(scale=2.0), 1, max_val))

    def _sample_damage(self, max_val: float) -> float:
        """Return a log-normal damage value between 0 and max_val."""
        if self.rng.random() < 0.30:
            return 0.0
        raw = self.rng.lognormal(mean=7.0, sigma=2.5)
    ...
    def _sample_magnitude(self, event_type: str) -> tuple[float | None, str | None]:
        ...
        if event_type == "TORNADO":
            ...
            mag = float(np.clip(self.rng.uniform(65.0, 300.0), 65.0, 300.0))
            return round(mag, 1), "MPH"
        if event_type == "HAIL":
            # Hail diameter in inches
            mag = float(np.clip(self.rng.exponential(scale=0.75) + 0.5, 0.5, 5.0))
            return round(mag, 2), "INCHES"
        if event_type == "THUNDERSTORM_WIND":
            mag = float(np.clip(self.rng.normal(55.0, 15.0), 25.0, 120.0))
            return round(mag, 1), "MPH"
        if event_type in ("HURRICANE", "TROPICAL_STORM"):
            mag = float(np.clip(self.rng.normal(90.0, 30.0), 39.0, 185.0))
            return round(mag, 1), "MPH"
