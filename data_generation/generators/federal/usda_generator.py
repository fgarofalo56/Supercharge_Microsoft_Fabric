...
class USDAGenerator(BaseGenerator):
    ...
    def _generate_crop_production_record(self) -> dict[str, Any]:
        ...
        year = int(self.rng.integers(2015, 2026))  # 2015–2025 inclusive

        # Geography
        state_fips = str(self.rng.choice(_FIPS_KEYS))
        state_name = STATE_FIPS[state_fips]
        county_fips: str | None = None
        county_name: str | None = None
        if agg_level == "COUNTY":
            # Generate a plausible 5-digit FIPS (state prefix + 3-digit county code)
            county_suffix = f"{self.rng.integers(1, 200):03d}"
        ...
        value = round(float(self.rng.uniform(meta["low"], meta["high"])), 2)

        # CV% applies to survey estimates; not applicable for Census
        cv_percent: float | None = None
        if source_desc == "SURVEY" and self.rng.random() < 0.75:
            cv_percent = round(float(self.rng.uniform(1.0, 25.0)), 1)

        # Reference period: annual or a specific month
        use_month = self.rng.random() < 0.25
        reference_period_desc = str(self.rng.choice(MONTHS)) if use_month else "YEAR"
    ...
    def _generate_food_safety_record(self) -> dict[str, Any]:
        ...
        recall_seq = int(self.rng.integers(1, 500))
        ...
        if self.rng.random() < 0.80:
            est_num = int(self.rng.integers(100, 99999))
            establishment_number = f"EST. {est_num}"

        # Optional city (70 % present)
        city: str | None = self.faker.city() if self.rng.random() < 0.70 else None

        # Optional pounds recalled (90 % present, realistic range)
        pounds_recalled: float | None = None
        if self.rng.random() < 0.90:
            pounds_recalled = round(float(self.rng.uniform(100.0, 2_500_000.0)), 0)

        # Distribution scope
        distribution: str | None = None
        dist_roll = self.rng.random()
        if dist_roll < 0.50:
            distribution = "Nationwide"
        elif dist_roll < 0.85:
            # 2–6 state abbreviations
            n_states = int(self.rng.integers(2, 7))
