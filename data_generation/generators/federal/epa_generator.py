...
class EPAGenerator(BaseGenerator):
    ...
    * ``"water_quality"`` – SDWIS public water system sample results
    ...
    def _generate_air_quality_record(self) -> dict[str, Any]:
        ...
        parameter = str(self.rng.choice(parameters))
        ...
        concentration = round(
            float(self.rng.uniform(lo, lo + (hi - lo) * min(aqi_fraction * 1.5, 1.0))),
            4,
        )
        ...
        sample_duration: str | None = (
            str(self.rng.choice(durations, p=dur_weights))
        ...
        if sample_duration == "1 HOUR":
            ...
            start_hour = self.rng.choice([0, 8, 16])
            time_local = f"{start_hour:02d}:00"
        else:
            time_local = None if self.rng.random() > 0.3 else "00:00"

        # Location
        state_code = str(self.rng.choice(_STATE_CODES))
        county_code = f"{self.rng.integers(1, 999):03d}"
        site_num = f"{self.rng.integers(1, 9999):04d}"
        site_id = f"{state_code}-{county_code}-{site_num}"

        lat = round(float(self.rng.uniform(25.0, 49.0)), 6)
        lon = round(float(self.rng.uniform(-125.0, -67.0)), 6)
        ...
        record: dict[str, Any] = {
            ...
            "county_name": (
                self.faker.city() + " County" if self.rng.random() > 0.1 else None
            ),
            "cbsa_name": (
    ...
    def _realistic_aqi(self) -> int:
        ...
        if bucket < 0.55:
            return int(self.rng.integers(0, 51))
        elif bucket < 0.85:
            return int(self.rng.integers(51, 101))
        elif bucket < 0.95:
            return int(self.rng.integers(101, 151))
        elif bucket < 0.99:
            return int(self.rng.integers(151, 201))
        else:
            return int(self.rng.integers(201, 501))
    ...
    def _generate_water_quality_record(self) -> dict[str, Any]:
        ...
        contaminant = str(self.rng.choice(list(CONTAMINANT_SPECS.keys())))
        ...
        population_served = max(
            100,
            population_served + int(self.rng.integers(-50, 50)),
        )
        ...
        record: dict[str, Any] = {
            "record_id": self.generate_uuid(),
            "system_id": "PWS" + f"{self.rng.integers(1000000, 9999999):07d}",
