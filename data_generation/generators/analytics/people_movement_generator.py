...
class PeopleMovementGenerator(BaseGenerator):
    ...
    def generate_record(self) -> dict[str, Any]:
        ...
        sensor_idx = int(self.rng.integers(0, len(self._sensors)))
        ...
        person_count = int(self.rng.integers(pc_low, pc_high + 1))
        ...
        dwell_time_seconds = round(float(self.rng.uniform(dw_low, dw_high)), 1)

        # Velocity – normally distributed around typical walking speed
        velocity_raw = float(self.rng.normal(1.2, 0.4))
        velocity_mps = round(max(0.0, min(velocity_raw, 2.5)), 2)

        # Coordinates (normalized 0-100 within zone)
        x_coordinate = round(float(self.rng.uniform(0.0, 100.0)), 2)
        y_coordinate = round(float(self.rng.uniform(0.0, 100.0)), 2)

        # Floor level (use zone default most of the time)
        floor_level = zone_cfg["floor"]

        # Heat map cell
        col = str(self.rng.choice(HEAT_MAP_COLUMNS))
        row = int(self.rng.choice(HEAT_MAP_ROWS))
        ...
        if queue_detected:
            queue_length = int(self.rng.integers(2, 26))
            queue_wait_minutes = round(float(self.rng.uniform(1.0, 45.0)), 1)
        ...
        if sensor_type == "wifi_probe":
            ...
            signal_strength_dbm = int(self.rng.integers(-90, -29))
            battery_level = int(self.rng.integers(10, 101))
