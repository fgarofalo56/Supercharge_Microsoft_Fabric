...
class GeolocationGenerator(BaseGenerator):
    ...
    def __init__(
        ...
        for _i in range(NUM_DEVICES):
            dev_id = f"DEV-{self.rng.integers(10_000_000, 99_999_999):08d}"
    ...
    def generate_record(self) -> dict[str, Any]:
        ...
        device_id = str(self.rng.choice(self._device_ids))
        ...
        accuracy_meters = round(float(self.rng.uniform(acc_low, acc_high)), 2)

        # Determine indoor vs outdoor (~50% each)
        is_indoor = bool(self.rng.random() < 0.50)

        # Coordinates: Las Vegas base + small random offset
        latitude = round(
            BASE_LATITUDE
            + float(
                self.rng.uniform(-COORDINATE_OFFSET_RANGE, COORDINATE_OFFSET_RANGE)
            ),
            6,
        )
        longitude = round(
        ...
        if not is_indoor:
            altitude_meters = round(float(self.rng.uniform(600.0, 650.0)), 1)

        # Speed: stationary (~40%), walking, or vehicle
        speed_mps: float | None
        heading_degrees: float | None
        speed_roll = float(self.rng.random())
        if speed_roll < 0.40:
            # Stationary
            speed_mps = 0.0
            ...
            speed_mps = round(float(self.rng.uniform(0.0, 2.0)), 2)
            heading_degrees = round(float(self.rng.uniform(0.0, 360.0)), 1)
        else:
            # Vehicle speed (5–15 m/s for shuttles/valet, up to 30 m/s)
            if device_type in ("vehicle_gps", "shuttle_tracker", "valet_tag"):
                speed_mps = round(float(self.rng.uniform(5.0, 30.0)), 2)
            else:
                speed_mps = round(float(self.rng.uniform(2.0, 15.0)), 2)
            heading_degrees = round(float(self.rng.uniform(0.0, 360.0)), 1)

        # H3 index: present ~70% of the time (15-character hex string)
        h3_index: str | None = None
        if self.rng.random() < 0.70:
            h3_raw = self.rng.integers(0, 2**60, dtype=np.int64)
        ...
        if self.rng.random() < 0.40:
            geofence_id = str(self.rng.choice(self._geofence_ids))
            ...
            if geofence_event == "dwell":
                geofence_dwell_seconds = int(self.rng.integers(30, 7201))

        # Nearest POI
        poi_name = str(self.rng.choice(POI_NAMES))
        poi_distance_meters = round(float(self.rng.uniform(1.0, 500.0)), 1)

        # Indoor positioning
        floor_level: int | None = None
        indoor_zone: str | None = None
        if is_indoor:
            floor_level = int(self.rng.integers(1, 31))
            indoor_zone = str(self.rng.choice(INDOOR_ZONES))

        # Proximity trigger: ~15% of patron_app events
        proximity_trigger: str | None = None
        if device_type == "patron_app" and self.rng.random() < 0.15:
            proximity_trigger = str(self.rng.choice(PROXIMITY_TRIGGERS))
